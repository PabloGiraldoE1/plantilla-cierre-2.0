...
import { GroqError, GroqService } from '../../services/groq';
...
export class FormularioIncidente implements OnInit, OnDestroy {
  ...
  /** Define el icono y el color del toast. */
  tipoToast: 'exito' | 'aviso' = 'exito';
  ...
  // Mejora de texto con IA: se aplica automáticamente al generar el texto
  private readonly groq = inject(GroqService);
  readonly iaDisponible = this.groq.estaConfigurado;
  /** true mientras Groq reescribe la descripción, antes de armar el texto final. */
  readonly generandoTexto = signal<boolean>(false);
  ...
  ordenModalRaizales = signal<{ col: 'numero' | 'tipo' | 'descripcion' | 'uso'; dir: 1 | -1 }>({
    col: 'numero',
    dir: 1,
  });
  ...
  raizalesModal = computed(() => {
    ...
    const lista = busqueda
      ? this.raizalesDisponibles().filter(
          (r) =>
            r.numero_historia.toLowerCase().includes(busqueda) ||
            r.descripcion.toLowerCase().includes(busqueda),
        )
    ...
    return [...lista].sort((a, b) => {
      switch (col) {
        case 'numero':
          return dir * (Number(a.numero_historia) - Number(b.numero_historia));
        case 'tipo':
          return dir * a.tipo.localeCompare(b.tipo);
        case 'descripcion':
          return dir * a.descripcion.localeCompare(b.descripcion);
        case 'uso':
          return dir * ((a.usado_contador || 0) - (b.usado_contador || 0));
      }
    });
  });
  ...
  causasError = [
    {
      value: '1. Capacitación - Tiene la opción pero no sabe cómo hacerlo',
      label: '1. Capacitación - Tiene la opción pero no sabe cómo hacerlo',
    },
    {
      value: '2. Capacitación - Desconoce el proceso de Negocio',
      label: '2. Capacitación - Desconoce el proceso de Negocio',
    },
    {
      value: '3. Capacitación - Desconoce el uso del aplicativo',
      label: '3. Capacitación - Desconoce el uso del aplicativo',
    },
    {
      value: '4. Mejoras - Redefinición de Políticas y/o Procesos de negocio',
      label: '4. Mejoras - Redefinición de Políticas',
    },
    {
      value: '5. Mejoras - Se requiere una nueva funcionalidad',
      label: '5. Mejoras - Nueva funcionalidad',
    },
    { value: '6. Errores de la aplicación', label: '6. Errores de la aplicación' },
    {
      value: '7. Perfilacion / Accesos - El usuario no cuenta con los permisos',
      label: '7. Perfilación / Accesos',
    },
    { value: '8. Degradación de Servicios', label: '8. Degradación de Servicios' },
    { value: '9. Datos errados', label: '9. Datos errados' },
    {
      value: '10. Solucionado sin acciones - Fue un error Momentaneo',
      label: '10. Error Momentáneo',
    },
    {
      value: '11. Modificacion Dato - Se modifica un Valor el cual era incorrecto',
      label: '11. Modificación Dato',
    },
    {
      value: '12. Modificacion De Codigo - Se Modifica Algo del codigo',
      label: '12. Modificación De Código',
    },
    { value: '13. Usuario desincronizado', label: '13. Usuario desincronizado' },
    {
      value: '14. No detectada (Usuario ya no tiene póliza para confirmar)',
      label: '14. No detectada',
    },
  ...
  // ===== GENERAR TEXTO =========================================
  async generarTexto(): Promise<void> {
    ...
    const descripcion = await this.mejorarDescripcionSiAplica(v.descripcionSolucion);
...
* Descripción de Solución: ${descripcion}
...
* Confirmar Operatividad del Usuario Afectado: ${v.confirmacionUsuario}`.trim();
  ...
  /**
   * Mejora la redacción con Groq cuando está configurado. Si Groq no está
   * disponible o falla, devuelve el texto original tal cual escribió el
   * analista: nunca bloquea la generación del cierre.
   */
  private async mejorarDescripcionSiAplica(descripcionOriginal: string): Promise<string> {
    const original = (descripcionOriginal || '').trim();
    if (!this.iaDisponible || !original) return original;

    this.generandoTexto.set(true);
    try {
      return await this.groq.mejorarTexto(original);
    } catch (error) {
      const mensaje =
        error instanceof GroqError
          ? error.message
          : 'No se pudo mejorar el texto; se usó la redacción original.';
      this.showToast(mensaje);
      return original;
    } finally {
      this.generandoTexto.set(false);
    }
  }
  ...
  guardarIncidente(): void {
    ...
    this.showToast('Incidente guardado exitosamente', 'exito');
  ...
  copiarAlPortapapeles(): void {
    ...
    navigator.clipboard.writeText(this.textoGenerado).then(() => {
      this.showToast('Texto copiado al portapapeles', 'exito');
    });
  }
  ...
  showToast(message: string, tipo: 'exito' | 'aviso' = 'aviso'): void {
  ...
  ocultarSugerenciasRaizal(): void {
    setTimeout(() => {
      ...
      const esValida = this.raizalesDisponibles().some(
        (r) => formValue === `${r.numero_historia} - ${r.tipo} - ${r.descripcion}`,
      );
      if (!esOTRO && !esValida) {
        this.formulario.patchValue({ huRaizal: '' }, { emitEvent: false });
        this.busquedaRaizal.set('');
        this.raizalSeleccionada.set('');
        this.showToast('Debes seleccionar una raizal del listado o elegir "OTRO"');
      }
    }, 250);
  }
  ...
  confirmarCapacitacion(esCapacitacion: boolean): void {
    ...
    if (esCapacitacion) {
      ...
      this.formulario
        .get('huRaizalOtro')
        ?.setValidators([Validators.required, Validators.minLength(PREFIJO_FALLA.length + 3)]);
    }
    this.formulario.get('huRaizalOtro')?.updateValueAndValidity();
  }
  ...
  // ===== MODAL AZURE DEVOPS ===================================
  cerrarModalAzureDevops(): void {
    this.mostrarModalAzureDevops.set(false);
  }
